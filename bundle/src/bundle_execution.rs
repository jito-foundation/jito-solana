...
pub struct BundleTransactionsOutput<'a> {
    // TODO (LB): review all these public facing points
    pub transactions: Vec<&'a RuntimeTransaction<SanitizedTransaction>>,
    ...
    pub pre_tx_execution_accounts: Vec<Option<Vec<(Pubkey, AccountSharedData)>>>,
    pub post_tx_execution_accounts: Vec<Option<Vec<(Pubkey, AccountSharedData)>>>,
}
...
impl<'a> BundleTransactionsOutput<'a> {
    ...
    pub fn balance_collector(&self) -> &Option<BalanceCollector> {
        &self.load_and_execute_transactions_output.balance_collector
    }
    ...
    while chunk_start != bundle.transactions.len() {
        ...
        bundle_transaction_results.push(BundleTransactionsOutput {
            transactions: batch
                ...
                .iter()
                .zip(batch.lock_results())
                .filter(|(_tx_, lock_results)| lock_results.is_ok())
                .map(|(tx, _)| tx)
                .collect(),
            load_and_execute_transactions_output,
