...
def host(phase="baseline"):
    ...
    facts("host")
    expected = int(phase.rsplit("-", 1)[1]) if phase.startswith(("matrix-", "validate-", "affinity-")) else None
    ...
    if phase.startswith(("matrix", "affinity")):
    ...
    image = "anzaxyz/ci@sha256:13a59d8688bd940b5ea6b00603d81bf8341af5e8747bfb484de7fc5dd4e013c6"
    result = subprocess.run(["ci/docker-run.sh", image, *command], check=False)
    image_facts = capture(["docker", "image", "inspect", image,
                           "--format", "{{json .RepoDigests}}"])
    (OUT / "image.json").write_text(json.dumps(
        {"requested_image": image, "inspection": image_facts}, indent=2))
...
ARTIFACTS = [
    ...
    "target/lc2-matrix/*/*.xml.gz", "target/lc2-matrix/*/*.tar.gz",
    "target/lc2-matrix/*/*.patch",
...
def pipeline(phase="baseline"):
    phases = (["affinity-128"] if phase == "affinity-128" else
              ["validate-48"] if phase == "validation-short" else
