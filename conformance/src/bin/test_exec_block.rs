use {clap::Parser, prost::Message, protosol::protos::BlockFixture, std::path::PathBuf};
...
fn exec(input: &PathBuf) -> bool {
    ...
    let Ok(fixture) = BlockFixture::decode(&blob[..]) else {
    ...
    let effects = agave_conformance::block::execute_block_proto(&context);
