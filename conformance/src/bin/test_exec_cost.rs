use {clap::Parser, prost::Message, protosol::protos::CostFixture, std::path::PathBuf};
...
fn exec(input: &PathBuf) -> bool {
    ...
    let Ok(fixture) = CostFixture::decode(&blob[..]) else {
    ...
    let effects = agave_conformance::cost::execute_cost(&context);
