use {clap::Parser, prost::Message, protosol::protos::ShredParseFixture, std::path::PathBuf};
...
fn exec(input: &PathBuf) -> bool {
    ...
    let Ok(fixture) = ShredParseFixture::decode(&blob[..]) else {
    ...
    let effects = agave_conformance::shred::execute_shred_parse(&context);
