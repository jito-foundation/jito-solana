    ...
    fn parse_batch(
        ...
        for (index, verified_packet) in verified_batch.iter_mut().enumerate() {
            ...
            if bank_checks_enabled {
                ...
                let ((check_results, _), duration_us) =
                    measure_us!(working_bank.check_transactions_external(
        ...
        sigverify_thread_pool.install(|| {
            packet_data
                ...
                .map(|data| {
                    ...
                    let packet_data_size_limit = if is_v1 {
                    ...
                    let (is_simple_vote_transaction, is_valid) =
                        verify_transaction_view(&view, false);
                    if !is_valid {
                        return Err(PacketVerificationError::Failed);
                    }
...
pub(super) mod tests {
    ...
    fn v1_transaction_with_wire_size(
        ...
        let base_message = v1::Message::try_compile_with_config(
            &payer.pubkey(),
            &[base_instruction],
            blockhash,
    ...
    fn test_batch_verify_tx_v1_size_boundaries() {
        ...
        for target_size in [
            ...
            let mut metrics = BamReceiveAndBufferMetrics::default();
            let (results, _batch_stats) = run_batch_verify(
        ...
        for decision in [
            ...
            for (initial, state, published) in [
                ...
                let batches = (70_001..=70_002)
                    ...
                    .collect();
                // Prepare real parsed work synchronously: this test controls delivery at
                // the receiver boundary, without a deadline on instrumented sigverify.
                let mut metrics = BamReceiveAndBufferMetrics::default();
                let (verified, _) = run_batch_verify(batches, replacement.slot(), &mut metrics);
                let batches = verified
                    .into_iter()
                    .map(|batch| {
                        let (mut packets, revert_on_error, seq_id, max_schedule_slot) =
                            batch.unwrap();
