...
pub fn convert_txn_error_to_proto(err: TransactionError) -> TransactionErrorReason {
    match err {
        ...
        // No dedicated proto variant; report the instructions-sysvar overflow as a
        // sanitization failure (it is a sanitize-time size error).
        TransactionError::InstructionsSysvarOverflow => TransactionErrorReason::SanitizeFailure,
    }
}
