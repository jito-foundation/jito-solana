...
pub fn convert_txn_error_to_proto(err: TransactionError) -> TransactionErrorReason {
    match err {
        ...
        // `TransactionError` is `#[non_exhaustive]`; `BailOut` and future variants have
        // no proto reason yet, so report them as the nearest "not committed" reason.
        _ => TransactionErrorReason::CommitCancelled,
    }
}
