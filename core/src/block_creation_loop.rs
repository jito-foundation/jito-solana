...
fn handle_parent_ready(
    ...
    *block_timer = leader_window_info.block_timer;
    ...
    // The record receiver is already shut down and drained. Wait for commits of
    // admitted records before purging; later records for this bank are rejected.
    bank.wait_for_inflight_commits();
...
mod tests {
    ...
    fn test_sad_leader_handover(optimistic_parent_slot: Slot, expired_parent_ready: bool) {
        ...
        *ctx.highest_parent_ready.write().unwrap() = (
        ...
        // Both batch types are valid on the optimistic fork. The atomic batch's
        // second transaction and the ordinary batch use its unique blockhash, so
        // the replacement must reject both without committing the valid prefix.
        // Same-slot banks share accounts-db storage, so use blockhashes to
        // distinguish the forks. Hold both batches until ParentReady resolves it.
        ...
        assert_eq!(block_timer, parent_ready_started_at);
        assert!(
            record_sender
                .try_send(Record::new(
                    vec![Hash::new_unique()],
                    vec![vec![versioned_transfer(5)]],
                    optimistic_bank_id,
                    true,
                ))
                .is_err()
        ...
        for (seq_id, batch) in [(71, None), (72, None), (73, Some(control))] {
            ...
            for record in ctx.record_receiver.inner().try_iter() {
                ...
                ctx.poh_recorder
                    .write()
                    .unwrap()
                    .record(record.bank_id, record.mixins, record.transaction_batches)
                    .unwrap();
            }
