...
impl BundleConsumer {
    ...
    fn execute_and_commit_transactions_locked(
        ...
        let mut entry_bytes =
            SERIALIZED_ENTRIES_OVERHEAD.saturating_mul(batch.sanitized_transactions().len() as u64);
        let (processed_transactions, prepare_record_transactions_us) = measure_us!(
            processing_results
                .iter()
                .zip(batch.sanitized_transactions())
                .filter_map(|(processing_result, tx)| {
                    if processing_result.was_processed() {
                        entry_bytes = entry_bytes.saturating_add(tx.serialized_size() as u64);
