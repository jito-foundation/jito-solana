...
pub fn proto_packets_to_packet_batch<I>(packets: I) -> PacketBatch
where
    I: IntoIterator<Item = jito_protos::proto::packet::Packet>,
{
    // Convert proto packets to regular Packets
    let packets: Vec<Packet> = packets.into_iter().map(proto_packet_to_packet).collect();

    PinnedPacketBatch::new(packets).into()
}
