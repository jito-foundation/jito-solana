...
pub async fn generate_auth_tokens(
    ...
    let challenge_response = auth_service_client
        ...
        .map_err(|e: Status| {
            if e.code() == Code::PermissionDenied {
                ...
                ProxyError::AuthenticationError { code: e.code(), message: e.message().to_string() }
            }
        })?;
    ...
    let auth_tokens = auth_service_client
        ...
        .await
        .map_err(|e| ProxyError::AuthenticationError { code: e.code(), message: e.message().to_string() })?;
...
pub async fn maybe_refresh_auth_tokens(
    ...
    if should_generate_new_tokens {
        ...
        .map_err(|e| ProxyError::MethodError { code: tonic::Code::Unknown, message: e.to_string() })?;
