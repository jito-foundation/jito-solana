...
#[test_case(true, false, false, false; "wrong_parent_correct_child")]
#[test_case(false, false, false, false; "correct_parent_wrong_child")]
#[test_case(true, true, false, false; "parent_repaired_before_ancestor_reply")]
#[test_case(false, false, true, false; "pruned_request_child_now_dead")]
#[test_case(true, true, true, false; "pruned_parent_reply_child_now_dead")]
#[test_case(false, false, false, true; "epoch_sample_only_wrong_child")]
#[test_case(true, false, false, true; "epoch_sample_only_wrong_parent")]
fn test_dead_duplicate_repair_waits_for_ancestor_sample(
    wrong_parent: bool,
    stale_parent_sample: bool,
    pruned_request: bool,
    epoch_sample_only: bool,
) {
    ...
    let mut duplicate_confirmed_slots = if epoch_sample_only {
        DuplicateConfirmedSlots::default()
    } else {
        DuplicateConfirmedSlots::from([(2, child_hash)])
    };
    ...
    // A weak-only parent mismatch is first discovered by the actual ancestry
    // sample below. The child has no independently confirmed repair target yet.
    if !(epoch_sample_only && wrong_parent) {
        // Preserve the first immediate attempt, then return the same failing shreds.
        if epoch_sample_only {
            // Use the actual reference bank hashes and ancestor-response selection.
            // No direct duplicate confirmation is delivered for the child.
            let initial_sample = sample(child_hash);
            assert_eq!(initial_sample.slot_to_repair, (2, child_hash));
            apply_sample(
                initial_sample,
                &progress,
                &mut fork_choice,
            ...
            assert!(duplicate_confirmed_slots.is_empty());
        } else {
            check_state(
                2,
        ...
        for _ in 0..=MAX_REPAIR_RETRY_LOOP_ATTEMPTS {
            ...
            let mut states = vec![SlotStateUpdate::Dead(dead_state)];
            if !epoch_sample_only {
                states.push(SlotStateUpdate::DuplicateConfirmed(
                    DuplicateConfirmedState::new_from_state(child_hash, || true, || None),
                ));
            }
            for state in states {
                check_state(
    ...
    if !wrong_parent && !epoch_sample_only {
    ...
    let sampled_slot = first_sample.slot_to_repair.0;
    let attempts_before_sample = repair_counter
        .get(&sampled_slot)
        .copied()
        .unwrap_or_default();
    apply_sample(
    ...
    assert_eq!(
        repair_counter.get(&sampled_slot),
        Some(&attempts_before_sample.checked_add(1).unwrap())
    );
    ...
    if !repair_parent {
        ...
        let attempts_before_same_sample = repair_counter[&2];
        install_dead_child(&mut progress);
        apply_sample(
            sample(child_hash),
            &progress,
        ...
        assert_eq!(dumped_receiver.try_recv().unwrap(), vec![(2, child_hash)]);
        assert_eq!(
            repair_counter.get(&2),
            Some(&attempts_before_same_sample.checked_add(1).unwrap())
        );
    }

    let counters_before_replay = repair_counter.clone();
    ...
    if epoch_sample_only {
        // A weaker sample can recover the correct bank without promoting either
        // the parent or child to strong duplicate confirmation.
    ...
    assert!(duplicate_slots_to_repair.is_empty());
    if epoch_sample_only {
        // Existing policy retains retry history until strong confirmation or root
        // cleanup. A weaker matching sample must not erase that history.
        assert_eq!(repair_counter, counters_before_replay);
    } else {
