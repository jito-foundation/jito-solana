...
mod tests {
    ...
    #[test]
    fn jito_tonic_tls_call_sites_stay_centralized() {
        let repo_root = Path::new(env!("CARGO_MANIFEST_DIR")).parent().unwrap();
        let allowed = [
            "core/src/tonic_endpoint.rs",
            "storage-bigtable/src/bigtable.rs",
        ];
        let mut violations = Vec::new();

        for relative_path in ["core/src", "validator/src"] {
            collect_tonic_tls_violations(
                repo_root,
        ...
        assert!(
            violations.is_empty(),
            "Tonic TLS setup must go through core/src/tonic_endpoint.rs or a documented exception:\n{}",
            violations.join("\n")
        );
    }

    fn collect_tonic_tls_violations(
        repo_root: &Path,
        dir: &Path,
        allowed: &[&str],
        ...
        for entry in fs::read_dir(dir).unwrap() {
            ...
            let path = entry.path();
            if path.is_dir() {
                collect_tonic_tls_violations(repo_root, &path, allowed, violations);
                continue;
            }
            ...
            if allowed.iter().any(|allowed| *allowed == relative_str) {
                continue;
            }

            let contents = fs::read_to_string(&path).unwrap();
            for needle in ["ClientTlsConfig::new()", ".tls_config("] {
