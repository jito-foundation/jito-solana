...
fn main() -> Result<(), std::io::Error> {
    ...
    if std::env::var(PROTOC_ENVAR).is_err() {
        #[cfg(not(windows))]
        // SAFETY: Build scripts execute single-threaded at this point, and we set
        // the environment variable once before invoking any code that might spawn
        // threads or read `PROTOC`.
        unsafe {
    ...
    configure()
        ...
        .compile_protos(&protos, &[proto_base_path, proto_base_path_jds])
}
