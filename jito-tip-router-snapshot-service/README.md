...
`TipRouterEpochBoundaryFilter` performs coarse, stateless classification. It accepts a `Frozen` bank
whose epoch is greater than its parent's, deriving the parent's epoch from `parent_slot` so that a
boundary is still recognized when the first slots of an epoch are skipped.
...
one worker that collects metadata and atomically writes the artifact. The notification thread tracks
only the last claimed epoch and that one active worker; all expensive work remains off the Replay,
