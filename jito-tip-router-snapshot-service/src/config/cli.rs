...
mod tests {
    ...
    #[test]
    fn enabled_requires_output_dir() {
        let matches = app()
            .get_matches_from_safe(vec!["test", "--enable-tip-router-snapshot-service"])
            .unwrap();

        let err = config_from_matches(&matches).unwrap_err();

        assert_eq!(err.kind, ErrorKind::ArgumentNotFound);
    }

    #[test]
    fn secondary_flags_require_gate() {
        let err = app()
    ...
    #[test]
    fn enabled_config_parses_optional_fields() {
        ...
        let matches = app()
            .get_matches_from_safe(vec![
                ...
                "--tip-router-snapshot-output-dir",
                "/tmp/snapshots",
                "--tip-router-snapshot-ncn",
                &pubkey.to_string(),
                "--tip-router-snapshot-tip-router-program-id",
                &pubkey.to_string(),
                ...
                &pubkey.to_string(),
                "--tip-router-snapshot-max-candidates",
                "42",
            ])
            .unwrap();
        ...
        assert_eq!(
            ...
            Some(TipRouterSnapshotConfig {
                ...
                tip_distribution_program_id: Some(pubkey),
