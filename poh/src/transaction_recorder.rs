...
impl TransactionRecorder {
    ...
    pub fn record_batch(
        ...
        if !transaction_batches.is_empty() {
            let records: Vec<Record> = mixins
                .into_iter()
                .zip(transaction_batches)
                .map(|(mixin, transactions)| Record::new(mixin, transactions, bank_id))
                .collect();
            let (res, poh_record_us) = measure_us!(self.record_sender.try_send_batch(records));
            record_transactions_timings.poh_record_us = Saturating(poh_record_us);
    ...
    pub fn record_bundle(
        ...
        if !transactions.is_empty() {
            ...
            for tx in transactions {
                ...
                records.push(Record::new(hash, batch, bank_id));
            }
