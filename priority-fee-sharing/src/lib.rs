...
use anyhow::Result;
use log::info;
use solana_clock::{Epoch, DEFAULT_MS_PER_SLOT};
...
pub async fn share_priority_fees_loop(
    ...
    commission_bps: u64,
    minimum_balance: u64,
    priority_fee_distribution_program: solana_pubkey::Pubkey,
    csv_path: Option<String>,
) -> Result<()> {
    let mut leader_stats = LeaderStats::populate(keypair.pubkey(), rpc_client, csv_path).await?;
    ...
    loop {
        ...
        if epoch_info.epoch != leader_stats.epoch {
            ...
            leader_stats =
                LeaderStats::populate(keypair.pubkey(), rpc_client, leader_stats.csv_path.clone())
                    .await?;
        }
