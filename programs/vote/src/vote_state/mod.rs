...
pub fn process_vote_unfiltered(
    ...
    vote_slots.iter().for_each(|s| {
        vote_state.process_next_vote_slot(*s, epoch, current_slot, timely_vote_credits, pop_expired)
    });
    Ok(())
}
...
mod tests {
    ...
    fn build_vote_state(vote_slots: Vec<Slot>, slot_hashes: &[(Slot, Hash)]) -> VoteState {
        ...
        if !vote_slots.is_empty() {
            ...
            process_vote_unfiltered(&mut vote_state, &vote.slots, &vote, slot_hashes, 0, 0, true, true)
                .unwrap();
        }
