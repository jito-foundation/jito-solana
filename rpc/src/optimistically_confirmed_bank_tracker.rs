...
///
/// Filters are evaluated synchronously on the producer threads (replay, votor, the vote listener)
/// for every notification published, so implementations must be pure, non-blocking and cheap.
///
/// A filter sees only the [`BankNotification`]. The dependency work id is transport metadata for
/// the RPC tracker and must not influence delivery policy.
...
impl BankNotificationSender {
    ...
    /// Send only the notifications accepted by `filter` to `tx`.
...
mod tests {
    ...
    #[test]
    fn test_bank_notification_broadcaster_fans_out_and_skips_disconnected_subscribers() {
        let (first_sender, first_receiver) = bounded(1);
        let (second_sender, second_receiver) = bounded(2);
        let broadcaster = BankNotificationBroadcaster::new(vec![
            BankNotificationSender::new(first_sender),
            BankNotificationSender::new(second_sender),
        ]);
