...
impl JsonRpcRequestProcessor {
    ...
    fn get_slot(&self, config: RpcContextConfig) -> Result<Slot> {
        Ok(self.bank_from_commitment(config.commitment).slot())
    }

    fn get_block_height(&self, config: RpcContextConfig) -> Result<u64> {
        Ok(self.bank_from_commitment(config.commitment).block_height())
    }
    ...
    fn get_total_supply(&self, commitment: Option<CommitmentConfig>) -> Result<u64> {
        Ok(self.bank_from_commitment(commitment).capitalization())
    }
...
pub mod rpc_minimal {
    ...
    impl Minimal for MinimalImpl {
        ...
        fn get_epoch_info(
            ...
            let bank = meta.bank_from_commitment(config.unwrap_or_default().commitment);
...
pub mod utils {
    use {
        ...
        solana_runtime::bank::{
            AccountData, Bank, BundleSimulationResult, BundleSimulationSummary,
        },
        solana_sdk::{
            pubkey::Pubkey,
    ...
    fn try_encode_accounts(
        ...
        if let Some(accounts) = accounts {
            ...
            Ok(Some(
                accounts
                    .iter()
                    .map(|a| encode_account(&a.data, &a.pubkey, encoding, None))
    ...
    pub fn rpc_bundle_result_from_bank_result(
        ...
        for (i, res) in bank_result.transaction_results.into_iter().enumerate() {
            ...
            transaction_results.push(RpcSimulateBundleTransactionResult {
                ...
                return_data: res.return_data.map(|d| d.into()),
            });
        }
    ...
    impl Full for FullImpl {
        ...
        fn simulate_bundle(
            ...
            let bank = match config.simulation_bank.unwrap_or_default() {
                SimulationSlotConfig::Commitment(commitment) => {
...
pub mod tests {
    ...
    fn test_rpc_simulate_bundle_happy_path() {
        ...
        let b64_data = base64::encode(leader_account_data.data());
