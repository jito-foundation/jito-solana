use {clap::Parser, prost::Message, protosol::protos::TxnFixture, std::path::PathBuf};
...
fn exec(input: &PathBuf) -> bool {
    ...
    let Ok(fixture) = TxnFixture::decode(&blob[..]) else {
    ...
    let effects = solana_svm_conformance::txn::execute_txn_proto(context);
