...
fn retransmit(
    ...
    let cache: HashMap<Slot, _> = shred_slots
        ...
        .filter_map(|slot: Slot| {
            ...
            Some((
                slot,
                (slot_leader, cluster_nodes, include_bam_shred_receivers),
            ))
        })
        .collect();
...
fn retransmit_shred(
    ...
    let num_nodes = match cluster_nodes::get_broadcast_protocol(&key) {
        Protocol::QUIC => {
            let shred_bytes = shred.bytes.clone();
            let sent = turbine_addrs
                .iter()
                .filter_map(|&addr| {
                    quic_endpoint_sender
                        .try_send((addr, shred_bytes.clone()))
                        .ok()
                })
                .count();
            if !external_addrs.is_empty() {
                ...
                {
                    warn!(
                        "retransmit_to external multi_target_send error: {ioerr:?}, \
        ...
        Protocol::UDP => match socket {
            RetransmitSocket::Xdp(sender) => {
                ...
                if !send_addrs.is_empty() {
                    if let Err(e) = sender.try_send(key.index() as usize, send_addrs, shred.clone())
                    {
...
mod tests {
    ...
    fn test_retransmit_external_receivers_are_deduped_and_bam_gated() {
        ...
        let external_sender_socket = bind_to_localhost().unwrap();
