...
fn retransmit(context: &RetransmitContext, state: &mut RetransmitState) -> Result<(), ()> {
    ...
    let bam_shred_receiver_addresses = context.bam_shred_receiver_addresses.as_ref();
    let shredstream_receiver_address = context.shredstream_receiver_address.as_ref();
    let multicast_root_receiver_address = context.multicast_root_receiver_address.as_ref();
    let RetransmitState {
        stats,
        addr_cache,
        shred_buf,
            ...
            let include_bam_shred_receivers = !bam_shred_receiver_addresses_local.is_empty()
                ...
                && (1..=4).any(|offset| {
                    slot.checked_add(offset).is_some_and(|lookahead_slot| {
                        lookahead_slot % NUM_CONSECUTIVE_LEADER_SLOTS.get() as u64 == 0
    ...
    let num_nodes = match socket {
        ...
        RetransmitSocket::Socket(_) | RetransmitSocket::Multihomed { .. } => {
            ...
            if !external_addrs.is_empty()
                && let Err(SendPktsError::IoError(ioerr)) =
                    multi_target_send(external_sender_socket, &shred, &external_addrs)
            {
                warn!(
                    "retransmit_to external multi_target_send error: {ioerr:?}, {} destinations",
                    external_addrs.len()
                );
            }
            sent
    ...
    pub fn new(
        ...
        let external_sender_socket = Arc::new({
            ...
            if let Err(err) = socket.set_multicast_ttl_v4(64) {
                warn!("retransmit external_sender_socket: failed to set multicast TTL: {err}");
            }
            socket
        });
            ...
            |include_bam_shred_receivers,
             ...
             multicast_root_receiver_address: Option<SocketAddr>| {
                ...
                addr_cache.put(
                    ...
                    (
                        root_distance,
                        Arc::from([turbine_addr, duplicate_configured_addr] as [SocketAddr; 2]),
                    ),
                );
