    ...
    Arg::with_name("shred_retransmit_receiver_address")
        ...
        .help(
            "Validator will mirror TVU retransmit-stage shreds to these addresses in addition to \
             normal turbine operation. This applies only to shreds that enter retransmit; it \
             does not mirror this validator's own leader broadcast path. Pass this option \
             multiple times or use comma-separated ip:port or host:port entries. Hostnames \
             resolve to IPv4 addresses only. Up to 32 unique addresses are allowed. Set to empty \
             string to disable.",
        )
}
