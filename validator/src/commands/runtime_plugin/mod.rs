use {
    crate::{cli::DefaultArgs, commands::Result},
...
pub fn command(_default_args: &DefaultArgs) -> App<'_, '_> {
    SubCommand::with_name("runtime-plugin")
        ...
        .subcommand(SubCommand::with_name("list").about("List all current running runtime plugins"))
        .subcommand(
            SubCommand::with_name("unload")
                .about(
                    "Unload a particular runtime plugin. You must specify the runtime plugin name",
                )
        ...
        .subcommand(
            SubCommand::with_name("reload")
                .about(
                    "Reload a particular runtime plugin. You must specify the runtime plugin name \
...
pub fn execute(plugin_subcommand_matches: &ArgMatches, ledger_path: &Path) -> Result<()> {
    ...
    match plugin_subcommand_matches.subcommand() {
        ...
        ("reload", Some(subcommand_matches)) => {
            if let Ok(name) = value_t!(subcommand_matches, "name", String) {
                if let Ok(config) = value_t!(subcommand_matches, "config", String) {
                    println!(
                        "This command does not work as intended on some systems.To correctly \
                         reload an existing plugin make sure to:1. Rename the new plugin binary \
                         file.2. Unload the previous version.3. Load the new, renamed binary \
